...
**Lifecycle step:** 4 of 17 (UX companion to the technical design) · **Brief locked:** 2026-09-16 · **Variants:** next — `mockups/direction-variants.html` · **Chosen:** — (pending)
...
**Published:** links added here when built (direction variants · screens · tracker).
...
## Variant page (`mockups/direction-variants.html`) — round 1, to build
Four full-size directions of S5 (editor with the café site loaded, a section selected, inspector open) and S9 on a 390 px phone, behind an A/B/C/D tab strip, each demonstrating its motion candidate live:

| Direction | Chrome | Canvas | Idea |
|---|---|---|---|
| **A · Studio** | Dark ink chrome (`#1B1B2F`), light panels text, indigo accents — Figma-like | The site sits on a mid-grey desk with a soft shadow | The tool recedes; the site is the bright object |
| **B · Paper desk** | Warm off-white chrome, hairline borders, ink text | Site on a cream desk with the faint brand grid | Calm, print-like; the editor feels like a layout table |
| **C · Blueprint** | White chrome with the brand grid everywhere, indigo dashed guides, monospace labels | Site drawn on grid paper; selection = blue dashed | Engineering drawing; pairs with motion A |
| **D · Glass** | Translucent panels over a soft indigo→mint wash, blurred | Site floats on the wash | Modern-app look; loudest |

Each variant carries the same tokens for the *published site* (the café's own theme), so only the chrome changes between tabs.
