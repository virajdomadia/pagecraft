...
## One-liner
A Framer-lite for one-page websites: pick a template, edit sections inline with a teammate in the same page at the same time (live cursors, nothing ever lost), hit Publish and the site is live on its own subdomain — or its own domain — fast, SEO-clean, Lighthouse 100s, and yours to take away as plain files.
...
- The unique feature (v4): the site has reach beyond Pagecraft — your own domain from the same Publish button, and an honest export (static files, GitHub history, import back) that most builders refuse to give. Free to run.
...
### 3. Versions — base → mid → advanced → reach
Every project is cut base → mid → advanced (rule set 2026-09-15); Pagecraft adds a fourth — the one unique, free-to-run feature every project carries (Viraj, 2026-09-17). v1 alone is a complete, sellable page builder.
...
| **v4 Your domain, your code** (reach) | **Custom domains:** the café connects `kaapicorner.in` from Settings — guided DNS panel (the one record to add, live "checking…"), the domain is attached to the Vercel project through the Domains API, SSL is issued, `www` redirects, and the same middleware maps the host to the site; Publish stays one click. Pro only (this is what makes Pro worth ₹299). **Export:** *Download* = a zip of static HTML/CSS + images built by the same section renderer that serves `/s/{slug}` — byte-for-byte what Pagecraft serves, Lighthouse 100s included; *Push to GitHub* = the same files committed to a repo in the user's GitHub account (OAuth), re-committed on every publish, so the site has a history outside Pagecraft. **Import:** the zip carries `pagecraft.json` (the document export); drop it on the dashboard and the site is recreated, editable — export → import → identical site is the test | The document + renderer is a real core with reach beyond Pagecraft: your own address, your own files, no lock-in. Runs on free APIs only | 8 |
...
Shared stack from [`projects/README.md`](../README.md): `web/` Next.js App Router + Tailwind 4, `api/` FastAPI on Vercel (FastAPI preset), Neon Postgres, Upstash Redis (v2), Vercel Blob, own cookie-session auth (`pc_session`), Resend (v2), Razorpay subscriptions (v3), pydantic-ai (v3), Vercel Domains API + GitHub OAuth/API (v4, both free). New deps: `yjs` + `y-protocols` (awareness) + `@dnd-kit` in web; `pycrdt` + `redis-py` asyncio in api. Setup is the minimum to deploy both apps with plain CI (web typecheck + build, api ruff + pytest). OpenAPI → TS types generated by a script and committed; no CI freshness gate, no Sentry, no uptime monitor. Accounts and keys are created just-in-time in the plan row that first needs them.

Tests only where a demo bug would embarrass: two `Y.Text` edits at the same offset converge (pycrdt, two docs); published JSON equals the CRDT export; slug uniqueness; site membership 403; v2 Playwright two-page edit test; v3 subscription webhook replay; v4 export → import round trip and host → site resolution.
...
Free-form canvas positioning · multi-page sites · rich-text formatting inside text fields (plain text + line breaks only) · custom CSS · e-commerce sections · real-money billing · mobile editing (phone = preview only, "edit on desktop").
...
- v4: export → import reproduces the site exactly (tested); a custom domain resolves with SSL within 10 minutes of the DNS record going live; the exported `index.html` scores the same Lighthouse numbers as the hosted page.
...
- *A v4?* Yes (Viraj, 2026-09-17): every project carries one unique feature that is free to run. Rejected on the way: an AI collaborator that edits the document live (per-request token cost), Telegram command editing + shop menu board, edit-history replay, A/B branching. Chosen: **custom domains + export + import** — like Tripsmith's MCP server and Frontrow's app, it extends the finished core's reach instead of adding a gimmick, and it reverses v0's "no custom domains, no code export" deliberately.
